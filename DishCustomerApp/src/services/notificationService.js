...
export const notificationService = {
  ...
  sendNotificationWithRetry(targetId, title, message, data = {}, retries = 3) {
      ...
      const isImportant = title.includes("Payment") || title.includes("Request") || title.includes("New") || data.isApproval;
      const cacheKey = isImportant ? `${targetId}_${title}` : `${targetId}_${title}_${message}`;
      const now = Date.now();
      ...
      (async () => {
          ...
          while (attempt < retries) {
            try {
                ...
                attempt++;
                console.warn(`Notification send failed, attempt ${attempt} of ${retries}`);
                if (attempt >= retries) {
